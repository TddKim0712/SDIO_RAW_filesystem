...
#define RAW_LOG_FLAG_PAYLOAD_LINEAR_TEST        0x00000001UL
#define RAW_LOG_FLAG_PAYLOAD_DUMMY_SENSOR       0x00000002UL
#define RAW_LOG_FLAG_PAYLOAD_SENSOR_DMA         0x00000004UL

#define RAW_LOG_DUMMY_PACKET_MAGIC              0x44554D59UL /* "DUMY" */
...
typedef struct
{
    ...
    uint32_t packet_seq;
    uint32_t tick_ms;
    uint32_t writer_seq_snapshot;
    uint32_t ready_slots_snapshot;
    uint32_t dropped_packets_snapshot;
    uint32_t committed_blocks_snapshot;
    uint32_t reserved0;
    uint8_t  tail[32];
} raw_log_dummy_packet_t;
#pragma pack(pop)

typedef char raw_log_dummy_packet_size_check[
    (sizeof(raw_log_dummy_packet_t) == RAW_LOG_SENSOR_PACKET_BYTES) ? 1 : -1];
