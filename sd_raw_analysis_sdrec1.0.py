...
INITIAL_READ_DELAY_MS = 350
LEFT_PANE_RATIO = 0.60
LEFT_PANE_MIN_WIDTH = 560
RIGHT_PANE_MIN_WIDTH = 380
LAYOUT_REPAIR_DELAYS_MS = (50, 120, 250, 500, 1000, 1800)
...
RAW_LOG_FLAG_PAYLOAD_TABLE_SENSOR = 0x00000008

RAW_LOG_DUMMY_PACKET_MAGIC = 0x44554D59  # "DUMY"
RAW_LOG_WAVE_PACKET_MAGIC = 0x53494E45   # "SINE"
RAW_LOG_WAVE_PACKET_FMT_S16 = 1
RAW_LOG_WAVE_PACKET_MAX_SAMPLES = 20
...
PARSED_EXPORT_FIELDS = [
    ...
    "flags",
    "payload_kind",
    "payload_validation_kind",
    "payload_validation_ok",
    "payload_validation_detail",
    "packet_magic_first",
    "source_packet_seq_first",
    "source_tick_ms_first",
    "table_index_first",
    "sample_count_first",
    "sample_format_first",
    "payload_crc_ok",
...
def classify_payload_kind(flags: int) -> str:
    if (flags & RAW_LOG_FLAG_PAYLOAD_TABLE_SENSOR) != 0:
...
def _init_payload_validation_result(payload_kind: str) -> dict:
    return {
...
def validate_linear_payload(payload: bytes, seq: int, lba: int, payload_bytes: int) -> dict:
    result = _init_payload_validation_result("linear_test")
    mismatch_count, first_bad = count_test_pattern_mismatches(payload, seq, lba, payload_bytes)
    result.update({
        "payload_validation_kind": "linear_test",
        "payload_validation_ok": (mismatch_count == 0),
        "payload_validation_detail": "",
...
def validate_dummy_payload(payload: bytes, payload_bytes: int, packet_bytes: int) -> dict:
    result = _init_payload_validation_result("dummy_cpu")
    usable = payload[:min(len(payload), max(0, payload_bytes))]
    result["payload_validation_kind"] = "dummy_cpu_struct"

    if packet_bytes <= 0:
        result["payload_validation_ok"] = False
        result["payload_validation_detail"] = "packet_bytes_invalid"
        return result

    if (payload_bytes % packet_bytes) != 0:
        result["payload_validation_ok"] = False
        result["payload_validation_detail"] = "payload_bytes_not_multiple_of_packet_bytes"
        return result

    for off in range(0, len(usable), packet_bytes):
        magic, source_packet_seq, source_tick_ms = struct.unpack_from("<3I", usable, off)
        if off == 0:
            result["packet_magic_first"] = fmt_hex32(magic)
            result["source_packet_seq_first"] = source_packet_seq
            result["source_tick_ms_first"] = source_tick_ms
        if magic != RAW_LOG_DUMMY_PACKET_MAGIC:
            result["payload_validation_ok"] = False
            result["payload_validation_detail"] = f"bad_dummy_magic@{off}={fmt_hex32(magic)}"
            return result

    result["payload_validation_ok"] = True
    return result


def validate_wave_payload(payload: bytes, payload_bytes: int, packet_bytes: int, payload_kind: str) -> dict:
    result = _init_payload_validation_result(payload_kind)
    usable = payload[:min(len(payload), max(0, payload_bytes))]
    result["payload_validation_kind"] = f"{payload_kind}_struct"
    ...
    for off in range(0, len(usable), packet_bytes):
        magic, source_packet_seq, source_tick_ms, table_index, sample_count, sample_format = struct.unpack_from("<6I", usable, off)
        if off == 0:
            result["packet_magic_first"] = fmt_hex32(magic)
            result["source_packet_seq_first"] = source_packet_seq
            result["source_tick_ms_first"] = source_tick_ms
            result["table_index_first"] = table_index
            result["sample_count_first"] = sample_count
            result["sample_format_first"] = sample_format
        if magic != RAW_LOG_WAVE_PACKET_MAGIC:
            result["payload_validation_ok"] = False
            result["payload_validation_detail"] = f"bad_wave_magic@{off}={fmt_hex32(magic)}"
            return result
        if sample_format != RAW_LOG_WAVE_PACKET_FMT_S16:
            result["payload_validation_ok"] = False
            result["payload_validation_detail"] = f"bad_sample_format@{off}={sample_format}"
            return result
        if sample_count > RAW_LOG_WAVE_PACKET_MAX_SAMPLES:
            result["payload_validation_ok"] = False
            result["payload_validation_detail"] = f"bad_sample_count@{off}={sample_count}"
            return result
...
def validate_v3_payload(payload: bytes, seq: int, lba: int, payload_bytes: int, packet_bytes: int, flags: int) -> dict:
    payload_kind = classify_payload_kind(flags)
    if payload_kind == "linear_test":
        return validate_linear_payload(payload, seq, lba, payload_bytes)
    if payload_kind == "dummy_cpu":
        return validate_dummy_payload(payload, payload_bytes, packet_bytes)
    if payload_kind in ("wave_table", "wave_dma"):
        return validate_wave_payload(payload, payload_bytes, packet_bytes, payload_kind)

    result = _init_payload_validation_result(payload_kind)
    result["payload_validation_kind"] = "unknown"
    result["payload_validation_ok"] = ""
    result["payload_validation_detail"] = f"unsupported flags=0x{flags & 0xFFFFFFFF:08X}"
    return result
...
def parse_data_block_v3_info(data: bytes) -> dict | None:
    ...
    block_crc_ok = (block_crc32 == block_crc_calc)
    payload_validation = validate_v3_payload(
        usable_payload,
        seq,
        declared_lba,
        payload_bytes,
...
def format_parsed_info(info: dict) -> str:
    ...
    if kind == "superblock_v1":
        ...
        lines.extend([
            ...
            f"  payload_kind={info.get('payload_kind', '')}",
            f"  payload_validation_kind={info.get('payload_validation_kind', '')}",
            f"  payload_validation_ok={info.get('payload_validation_ok', '')}",
            f"  payload_validation_detail={info.get('payload_validation_detail', '')}",
            f"  packet_magic_first={info.get('packet_magic_first', '')}",
            f"  source_packet_seq_first={info.get('source_packet_seq_first', '')}",
            f"  source_tick_ms_first={info.get('source_tick_ms_first', '')}",
            f"  table_index_first={info.get('table_index_first', '')}",
            f"  sample_count_first={info.get('sample_count_first', '')}",
            f"  sample_format_first={info.get('sample_format_first', '')}",
            f"  payload_first_16={info.get('payload_first_16', '')}",
            f"  payload_last_16={info.get('payload_last_16', '')}",
            f"  test_pattern_ok={info.get('test_pattern_ok', '')}",
            f"  test_pattern_mismatch_count={info.get('test_pattern_mismatch_count', '')}",
...
def analyze_infos(infos: list[dict], stall_threshold_ms: int, assume_dummy_pattern: bool) -> tuple[str, list[dict]]:
    ...
    for info in data_infos:
        ...
        if info.get("kind") == "data_block_v3" and info.get("payload_validation_ok") is False:
            payload_validation_fail += 1
            events.append(make_event(
                "PAYLOAD_VALIDATE_FAIL",
                info["lba"],
                seq=seq,
                boot=boot,
                detail=f"{info.get('payload_validation_kind', '')}: {info.get('payload_validation_detail', '')}",
            ))

        if assume_dummy_pattern and info.get("payload_validation_kind") == "linear_test" and info.get("test_pattern_ok") is False:
            pattern_fail += 1
            events.append(make_event(
                "PATTERN_FAIL",
                info["lba"],
    ...
    if (payload_crc_fail + block_crc_fail + pattern_fail) > 0 and missing_seq_blocks == 0 and actual_lba_gap_count == 0:
        judgements.append("seq/LBA는 이어지는데 payload 내용 또는 구조 검증이 실패한다. 데이터 경로와 파서 기준을 같이 봐야 한다.")
    ...
    lines.append(f"  payload_validation_fail={payload_validation_fail}")
    if assume_dummy_pattern:
        linear_infos = [info for info in data_infos if info.get("payload_validation_kind") == "linear_test"]
        lines.append(f"  test_pattern_fail(linear_only)={pattern_fail} / {len(linear_infos)} ({pct_text(pattern_fail, len(linear_infos))})")
    else:
        lines.append("  test_pattern_fail=skipped")
    lines.append("")
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        self.analysis_summary_frame = None
        self.analysis_events_frame = None
        self.bottom_pane = None
        self.left_frame = None
        self.right_frame = None
        self.left_notebook = None
        self._layout_repair_job = None
    ...
    def _build_ui(self):
        ...
        left_frame = ttk.Frame(bottom_pane, width=860)
        right_frame = ttk.Frame(bottom_pane, width=580)
        ...
        ttk.Checkbutton(
            analysis_controls,
            text="Validate payload by flags",
        ...
        ttk.Label(
            analysis_controls,
            text="stall은 tick delta와 on-block cycle time 둘 다 본다. payload 검증은 flags에 따라 linear/dummy/wave 구조로 나눈다.",
            foreground="#555555",
        ...
        self.analysis_summary_frame = summary_frame
        self.analysis_events_frame = events_frame
        analysis_notebook.add(summary_frame, text="SUMMARY")
        analysis_notebook.add(events_frame, text="EVENTS")

        self.analysis_summary_text = ScrolledText(summary_frame, wrap="word", font=("Consolas", 10), height=12)
        ...
        events_tree = ttk.Treeview(events_frame, columns=columns, show="headings", height=12)
    ...
    def _desired_left_pane_width(self, total_width: int) -> int:
        total_width = max(int(total_width), LEFT_PANE_MIN_WIDTH + RIGHT_PANE_MIN_WIDTH)
        desired = int(total_width * LEFT_PANE_RATIO)
        desired = max(desired, LEFT_PANE_MIN_WIDTH)
        desired = min(desired, total_width - RIGHT_PANE_MIN_WIDTH)
        if desired <= 0:
            desired = total_width // 2
        return desired

    def _repair_pane_layout(self, force: bool = False) -> bool:
        if self.bottom_pane is None:
            return False
        try:
            self.root.update_idletasks()
            total = self.bottom_pane.winfo_width()
            if total < (LEFT_PANE_MIN_WIDTH + RIGHT_PANE_MIN_WIDTH):
                return False

            desired = self._desired_left_pane_width(total)
            current = self.bottom_pane.sashpos(0)

            left_bad = current < LEFT_PANE_MIN_WIDTH
            right_bad = (total - current) < RIGHT_PANE_MIN_WIDTH
            if force or left_bad or right_bad or abs(current - desired) > 120:
                self.bottom_pane.sashpos(0, desired)

            if self.left_frame is not None:
                self.left_frame.configure(width=desired)
            if self.right_frame is not None:
                self.right_frame.configure(width=max(RIGHT_PANE_MIN_WIDTH, total - desired))

            if self.left_notebook is not None:
                self.left_notebook.update_idletasks()
            return True
        except Exception:
            return False

    def _schedule_layout_repairs(self):
        self.root.after_idle(lambda: self._repair_pane_layout(force=True))
        for delay in LAYOUT_REPAIR_DELAYS_MS:
            self.root.after(delay, self._repair_pane_layout)

    def _schedule_single_layout_repair(self, delay_ms: int = 80):
        if self._layout_repair_job is not None:
            try:
                self.root.after_cancel(self._layout_repair_job)
            except Exception:
                pass
        self._layout_repair_job = self.root.after(delay_ms, self._run_scheduled_layout_repair)

    def _run_scheduled_layout_repair(self):
        self._layout_repair_job = None
        self._repair_pane_layout()

    def _on_root_configure(self, _event=None):
        self._schedule_single_layout_repair()
    ...
    def focus_analysis_summary(self):
        if self.analysis_notebook is not None and self.analysis_summary_frame is not None:
            self.analysis_notebook.select(self.analysis_summary_frame)
            self.analysis_notebook.update_idletasks()
        if self.analysis_summary_text is not None:
            self.analysis_summary_text.see("1.0")
            self.analysis_summary_text.update_idletasks()
        self.root.update_idletasks()
    ...
    def read(self):
        try:
            ...
            self._repair_pane_layout(force=True)
...
def main():
    ...
    root = tk.Tk()
    app = App(root)
    root.after(INITIAL_READ_DELAY_MS, app.read_initial_view)
    root.mainloop()
    return 0
